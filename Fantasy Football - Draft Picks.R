...
trade_data <- read.csv("C:/Users/alexc/Desktop/Fantasy Football/Trade Data/tradedata.csv")
fantasypros <- read.csv("C:/Users/alexc/Desktop/Fantasy Football/Data/FantasyPros_2020_Dynasty_ALL_Rankings.csv")

date_start <- as.Date("2019-07-01")
players_in_init_optimization <- 200
min_trades_for_val <- 5
average_player_value <- 20
export_data <- "Yes"

find_val <- function(char_expr, match_field) {
  x_start <- regexpr(char_expr, match_field) + nchar(char_expr) + 1
  x_string <- substr(match_field, x_start, x_start + 10)
  x_end = regexpr(",", x_string) - 1
  return(substr(x_string, 1, x_end))
}

trade_data <- trade_data %>%
  mutate(ppr = find_val("ppr", league_settings),
         is_dynasty = find_val("is_dynasty", league_settings),
         num_qbs = find_val("num_qbs", league_settings),
         num_teams = find_val("num_teams", league_settings))
...
trade_data_2 <- trade_data %>%
  ...
  filter(all_players != "[Dwayne Haskins,Allen Robinson]")
...
qb_model <- lm(trade_miss ~ ppr_type, data = filter(rel_data, position == "QB"))
rb_model <- lm(trade_miss ~ ppr_type, data = filter(rel_data, position == "RB"))
wr_model <- lm(trade_miss ~ ppr_type, data = filter(rel_data, position == "WR"))
te_model <- lm(trade_miss ~ ppr_type, data = filter(rel_data, position == "TE"))
pick_model <- lm(trade_miss ~ ppr_type, data = filter(rel_data, position == "Pick"))

d_ppr_type <- distinct(select(all_player_2, ppr_type))

league_settings <- d_ppr_type
...
adj_player_1 <- all_player_2 %>%
  left_join(settings_adjustments, by = c("s1_p1_position" = "position", "ppr_type")) %>%
  rename(s1_p1_adj = adjustment) %>%
  left_join(settings_adjustments, by = c("s1_p2_position" = "position", "ppr_type")) %>%
  rename(s1_p2_adj = adjustment) %>%
  left_join(settings_adjustments, by = c("s1_p3_position" = "position", "ppr_type")) %>%
  rename(s1_p3_adj = adjustment) %>%
  left_join(settings_adjustments, by = c("s2_p1_position" = "position", "ppr_type")) %>%
  rename(s2_p1_adj = adjustment) %>%
  left_join(settings_adjustments, by = c("s2_p2_position" = "position", "ppr_type")) %>%
  rename(s2_p2_adj = adjustment) %>%
  left_join(settings_adjustments, by = c("s2_p3_position" = "position", "ppr_type")) %>%
...
rankings_comparison <- rankings_combined %>%
  ...
  mutate(value_score = trade_value - fp_value,
